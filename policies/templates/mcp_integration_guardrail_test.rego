...
_mcp(server, pin, sev, verb) := {
	"tool_name": "some_tool",
	"mcp": {"server": server, "transport": "stdio", "surface": "tools/call", "pin_status": pin, "scan_severity": sev},
	"direction": "call",
...
test_a_quarantined_definition_blocks {
	g.decision == "block" with input as _mcp("demo", "quarantined", "none", "read")
	g.rule_id == "mcp_tool_not_approved" with input as _mcp("demo", "quarantined", "none", "read")
}
...
test_quarantine_wins_over_drift_so_one_call_gets_one_reason {
	...
	g.rule_id == "mcp_tool_not_approved" with input as _mcp("demo", "quarantined", "critical", "delete")
}

# ── per-integration scope ──────────────────────────────────────────────────────────────────────────

test_a_write_through_an_unapproved_integration_blocks {
	g.decision == "block" with input as _mcp("untrusted-kb", "pinned", "none", "write")
	g.rule_id == "mcp_unapproved_write_server" with input as _mcp("untrusted-kb", "pinned", "none", "write")
}

test_a_read_through_the_same_unapproved_integration_is_allowed {
	...
	g.decision == "allow" with input as _mcp("untrusted-kb", "pinned", "none", "read")
}
...
test_an_unclassified_verb_is_not_treated_as_a_write {
	...
	g.decision == "allow" with input as _mcp("untrusted-kb", "pinned", "none", "unknown")
}
...
test_answering_a_server_with_a_credential_blocks {
	inp := {
		"tool_name": "read_file",
		"mcp": {"server": "postgres-prod", "pin_status": "pinned", "scan_severity": "none"},
		"direction": "answer",
...
test_the_same_credential_on_the_CALL_plane_is_not_this_rule {
	...
	inp := {
		...
		"mcp": {"server": "mailer", "pin_status": "pinned", "scan_severity": "none"},
		"direction": "call",
...
test_every_non_allow_decision_carries_a_named_rule_and_a_real_reason {
	...
	inp := _mcp("demo", "quarantined", "none", "delete")
	g.decision == "block" with input as inp
