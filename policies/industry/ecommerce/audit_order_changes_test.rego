...
package norviq.industry.ecommerce.audit_order_changes_test

import rego.v1
import data.norviq.industry.ecommerce.audit_order_changes

test_triggers_expected_decision if {
    result := audit_order_changes.decision with input as {
        "tool_name": "order_tool",
        "tool_params": {"query": "order detected"},
...
test_sets_rule_id_and_reason if {
    ...
    rid := audit_order_changes.rule_id with input as trigger_input
    rsn := audit_order_changes.reason with input as trigger_input
    rid == "audit_order_changes"
