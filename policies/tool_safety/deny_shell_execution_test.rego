...
package norviq.tool_safety.deny_shell_execution_test

import rego.v1
import data.norviq.tool_safety.deny_shell_execution

test_triggers_expected_decision if {
    result := deny_shell_execution.decision with input as {
        "tool_name": "&&_tool",
...
test_sets_rule_id_and_reason if {
    ...
    rid := deny_shell_execution.rule_id with input as trigger_input
    rsn := deny_shell_execution.reason with input as trigger_input
    rid == "deny_shell_execution"
