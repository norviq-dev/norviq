...
package norviq.access_control.deny_admin_tools_test

import rego.v1
import data.norviq.access_control.deny_admin_tools

test_triggers_expected_decision if {
    result := deny_admin_tools.decision with input as {
        "tool_name": "admin_tool",
        "tool_params": {"query": "admin detected"},
...
test_sets_rule_id_and_reason if {
    ...
    rid := deny_admin_tools.rule_id with input as trigger_input
    rsn := deny_admin_tools.reason with input as trigger_input
    rid == "deny_admin_tools"
