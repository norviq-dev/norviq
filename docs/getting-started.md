...
```bash
cosign verify ghcr.io/norviq-dev/charts/norviq:0.2.2 \
...
```bash
helm upgrade norviq oci://ghcr.io/norviq-dev/charts/norviq --version 0.2.2 -n norviq --reset-then-reuse-values --set webhook.injection.enabled=true
```
