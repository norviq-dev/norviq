...
Apply an overlay with `helm install norviq oci://ghcr.io/norviq-dev/charts/norviq --version 0.2.4 -n norviq -f values-prod.yaml`
...
```
helm install norviq oci://ghcr.io/norviq-dev/charts/norviq --version 0.2.4 -n norviq --create-namespace \
...
helm install norviq oci://ghcr.io/norviq-dev/charts/norviq --version 0.2.4 -n norviq \
...
`existingSecretKey` defaults to `url`. Keep `config.dbSslMode` at its derived `require` (any external/HA host) for a managed
...
| `config.dbSslMode` | `""` → derived | Empty derives from the datastore in use: `disable` for the **bundled** Postgres StatefulSet (no TLS listener), `require` for CloudNativePG HA and for any external/managed host. An explicit value always wins; `values-prod.yaml` pins `require`. It was formerly a hard `require`, which made a pure-defaults install crash-loop on `rejected SSL upgrade`. |
...
| `gracefulShutdown.preStopSleepSeconds` | `15` | `preStop` sleep on api/engine/webhook so the Service deregisters the endpoint before `SIGTERM`, draining in-flight requests during a rolling upgrade. `0` disables the hook.  The chart moved to 15 on measurement: at 3 a rolling restart refused 1 and 31 benign calls across two runs, at 15 it refused 0, 0 and 2 across three (`values.yaml` lines 160-161). |
...
| `webhook.injection.fallbackMode` | `allow` | **Data-plane** posture — what a governed agent does when the engine is unreachable (distinct from `failurePolicy`, which is the *admission* posture). Ships `allow`: the agent keeps working through an engine outage, and every unjudged call is attributed to the named `engine_unavailable_fallback` rule so it can be counted and alerted on. Set `block` to refuse instead. Note this applies only to a genuinely unreachable engine — a 4xx is the engine *answering*, and always blocks regardless of this setting. This row previously said the default was `block`, which overstated the shipped posture. |
...
| `config.rateLimit` | `60` | **Currently inert.** It renders `NRVQ_RATE_LIMIT`, but there is no `rate_limit` setting — the per-identity evaluator limit is `evaluator_rate_limit_per_window` (60 per `evaluator_rate_limit_window_s`, 60s). To actually change it, pass `NRVQ_EVALUATOR_RATE_LIMIT_PER_WINDOW`. Note this is the OPA-policy limit on evaluated tool calls; the separate HTTP-layer throttle in front of the API is `NRVQ_HTTP_RATE_LIMIT_*` — its ceilings have no chart key, but its client-IP derivation does (`config.httpRateLimitTrustedProxyHops`, below). |
| `config.httpRateLimitTrustedProxyHops` / `…Cidrs` | `1` / `["127.0.0.0/8","::1/128"]` | How the rate limiter derives the client IP. `X-Forwarded-For` is client-writable, so trusting its left-most entry lets a caller rotate the header and never fill a bucket (throttle bypass, worst on the pre-auth `/auth/login` route) — but ignoring it entirely collapses every caller behind a proxy into **one** bucket, so a single abuser throttles everyone. XFF is therefore believed only when **both** the TCP peer is in `…Cidrs` **and** the chain has at least `…Hops` entries, in which case the `N`th entry **from the right** is used (what your outermost trusted proxy observed; forged left-hand entries are ignored). Repeated headers are joined per RFC 7239, and the selected entry is port-stripped and validated as an IP — a non-IP falls back to the peer. The defaults match the shipped topology: `config.internalTls` runs an nginx in the api pod that sets `X-Forwarded-For $remote_addr` (it *replaces* the header), so the peer is loopback and there is exactly one unforgeable entry; a workload that bypasses it and hits the API port directly is not loopback, so its XFF is ignored. Raise `…Hops` only if you add more *appending* proxies, and widen `…Cidrs` to match — anything listed there can set any client's rate-limit identity. `0` disables XFF entirely. |
...
| `config.dbSslMode` | `""` → derived | See Production checklist. Derives to `disable` for the bundled (non-TLS) Postgres and `require` for HA/external; set explicitly to pin. |
...
| `auth.requireBoundAgentIdentity` | `true` | Ratchet for `/evaluate` identity binding. A credential that **carries** an `agent_class`/`spiffe_id` claim is always pinned to it (403 `NRVQ-AUTH-14019` on a mismatch) regardless of this flag — `agent_class` selects the enforced Rego program and `spiffe_id` keys trust + the `agent_frozen:` kill-switch. This governs credentials carrying **no** binding (sidecar tokens minted before this shipped, keys issued without an `agent_class`): `false` still accepts them, so an upgrade doesn't fail-closed every in-flight sidecar; `true` requires every non-admin caller to be bound (403 `NRVQ-AUTH-14020`). Flip it once sidecars are re-admitted (the webhook stamps `agent_class` from the pod's `norviq.io/agent-class` label) and workload keys re-issued. |
