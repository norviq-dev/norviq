...
- **Container images** are gated by Trivy (`build.yml` post-build scan on `main`); the SAST gate
  ...
  diff-aware against every PR — new HIGH/CRITICAL findings fail the build, never backlogged. semgrep's
  scan includes `ui/src`, so JS/TS is covered fail-closed; a dedicated `eslint-security` gate is a
  not-yet-enabled ratchet (no eslint config is wired into `ui/` yet, so its CI step is a non-blocking
  placeholder), not a live blocker (see [`security-baseline.md`](engineering/security-baseline.md) for
  the full triage rule and the whole-repo-scan ratchet plan).
