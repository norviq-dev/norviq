...
|------|------|--------------|-------|-------------------------------|----------------|
| 2026-07 | trivy (image) | `CVE-2026-39822` — Go stdlib `os.Root` symlink following (HIGH) | `.trivyignore.yaml`; present via the embedded Go runtime of the pinned `opa 1.18.0-static` binary in the api/engine images, and the webhook Go binary | Not on any Norviq code path: OPA is driven as a policy evaluator over an HTTP API with no untrusted-filesystem/symlink surface, and the webhook does no `os.Root` traversal. Fixed in Go 1.25.12 / 1.26.5 / 1.27.0-rc.2; the pinned OPA is built with Go 1.26.4, so we cannot clear it by bumping our own code. | Drop the `.trivyignore.yaml` line when an OPA release built with Go ≥ 1.26.5 is pinned. |
| 2026-07 | trivy (image) | `GHSA-hrxh-6v49-42gf` — grpc-go xDS RBAC authorization bypass + HTTP/2 Rapid Reset DoS + xDS parse panic (HIGH) | `.trivyignore.yaml`; present via `google.golang.org/grpc v1.81.1` statically linked into the pinned `opa 1.18.0-static` binary in the api/engine images | All three sub-issues need a running grpc-go **server**: the RBAC bypass and the crafted-`NOT`-rule panic need the xDS RBAC engine fed by an xDS control plane, and the Rapid Reset bypass needs grpc-go's HTTP/2 server transport. Norviq starts neither — the in-image binary is only forked as `opa eval` / `opa check`, and the sidecar's `opa run --server` is HTTP REST on `127.0.0.1` with no Envoy ext_authz plugin (we ship `-static`, not `-envoy`) and no OPA distributed tracing, so no gRPC listener and no xDS client exists. Fixed in grpc-go 1.82.1, but no OPA release through v1.18.2 has adopted it, so we cannot clear it by bumping. | Drop the `.trivyignore.yaml` line when an OPA release built against grpc-go ≥ 1.82.1 is pinned. |
| 2026-07 | trivy (image) | `CVE-2026-56852` — infinite loop in `golang.org/x/text/unicode/norm` on invalid input (HIGH) | `.trivyignore.yaml`, **scoped to `paths: [usr/local/bin/opa]`**; present via `golang.org/x/text v0.38.0` statically linked into the pinned `opa 1.18.0-static` binary in the api/engine images | OPA imports `x/text` in exactly one place — `internal/presentation`, `var TitleCase = cases.Title(language.Und, cases.NoLower)`; `x/text/cases` is what links `unicode/norm`. Every call site is `generateTableWithKeys`, which title-cases **CLI table headers**, and every argument is a compile-time literal (`"Metric"`, `"Value"`, `"Time"`, the fixed `statKeys` slice, `opa inspect`'s fixed columns). No request, policy or tool-call input reaches it, and that pretty-printed table path is CLI `--format=pretty` output, which neither `opa run --server` nor our `opa check` / `opa eval` subprocess calls produce. Fixed in x/text 0.39.0 (2026-06-30), but the newest OPA release v1.18.2 (2026-07-02) still requires v0.38.0 — only OPA's unreleased `main` has moved. **Our own** Go binary was fixed, not baselined: `webhook/go.mod` pins v0.39.0, which is why this row is path-scoped rather than global. | Drop the line when an OPA release requires x/text ≥ 0.39.0. |
