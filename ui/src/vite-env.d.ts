...
// The minimal editor-core entry (used by lib/monaco.ts to avoid bundling all built-in
...
interface ImportMetaEnv {
  ...
  // OIDC SSO. When VITE_OIDC_ISSUER + VITE_OIDC_CLIENT_ID are set, the console logs in via
  // Auth Code + PKCE instead of the dev token. VITE_OIDC_REDIRECT_URI defaults to origin/auth/callback.
  readonly VITE_OIDC_ISSUER?: string;
  readonly VITE_OIDC_CLIENT_ID?: string;
  readonly VITE_OIDC_REDIRECT_URI?: string;
  // Multi-cluster fleet. Set to a fleet-api hub URL to enable the cross-cluster Fleet view.
...
// Runtime config injected by the container entrypoint (config.js) — see ui/docker-entrypoint.sh.
interface Window {
  __NRVQ_CONFIG__?: {
    fleetApiUrl?: string;
    oidcIssuer?: string;
