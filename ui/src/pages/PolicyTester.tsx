...
export function PolicyTester() {
  ...
  const [namespaceSelection, setNamespaceSelection] = useState<string>(selectedNamespace || "default");
  ...
  const signalSource = result?.trustSignals ?? {
    ...
    param_entropy: 1,
    time_decay: 1,
    chain_depth: Math.max(0, Math.min(1, 1 - chainDepth / 5)),
    session_velocity: 1
  };
  ...
  async function evaluate() {
    ...
    try {
      ...
      const trustSignals = response.trust_signals ?? signalSource;
      const nextResult = {
              ...
              <div>
                ...
                <div className="stack" style={{ gap: 8 }}>
                  {SIGNAL_ORDER.map((signal) => {
                    const value = Math.max(0, Math.min(1, Number(signalSource[signal] ?? 0)));
                    return (
