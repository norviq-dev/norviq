...
//
// UI wiring + cosmetics E2E. Drives the REAL SPA (nginx) + API on the live kind
// cluster and asserts the EFFECT (not 200s) for the wiring fixes:
//   Graph default-hides synthetic/probe agents; include_synthetic brings them back.
//   Active-policy "matches" reflects the real governed-call count (not 0).
//   Version history survives a restart (rehydrated from policy_versions).
//   /agents parses namespace + agent_class from the SVID and returns last_seen.
//   /audit/stats exposes engine_errors as its own signal.
//   A policy apply re-stamps last_applied (even for unchanged content).
//   Intent drafts dedupe by class (one latest draft per ns/class).
...
test.describe("UI wiring — EFFECT proofs on the live console", () => {
  ...
  test("attack paths default-hide probe-rooted kill-chains", async ({ page }) => {
  ...
  test("real matches, agent SVID columns, and the engine_errors signal are wired", async ({ page }) => {
    ...
    expect(cs?.matches).toBeGreaterThan(0); // real governed-call count, not 0
    ...
    expect(withMeta.length).toBeGreaterThan(0); // ns/class parsed from the SVID
    expect((agents.body as any[]).some((a) => a.last_seen)).toBeTruthy(); // last_seen present

    const stats = await apiJson(page, "/api/v1/audit/stats?range=30d");
    expect(stats.body).toHaveProperty("engine_errors"); // distinct signal
    expect(typeof stats.body.engine_errors).toBe("number");
  });

  test("version history is present after the restart (rehydrated from policy_versions)", async ({ page }) => {
  ...
  test("a policy apply re-stamps last_applied even for unchanged content", async ({ page }) => {
