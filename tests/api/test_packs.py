...
class _FakeLoader:
    ...
    def get_current(self, ns, agent_class):
        # A distinctive per-namespace override rego so a cross-tenant leak is observable in the response body.
        return f"package norviq.override\n# secret-override-for:{ns}\n"
...
# --- EXHAUSTIVE-PERF-AUDIT: GET /policy-packs/override read-scoping (cross-tenant IDOR) ---

def _bearer(role: str, namespace: str) -> dict:
    return {"Authorization": f"Bearer {_token(role, namespace)}"}


def test_override_read_cross_tenant_is_403() -> None:
    # IDOR: the GET override read discarded the caller (`_ = user`) and filtered by the raw ?namespace query — a
    # viewer scoped to tenant-b could read DEFAULT's override rego (a tenant's authored policy). Must be 403 now.
    client, _, _ = _client(rows=[])
    resp = client.get("/api/v1/policy-packs/override?namespace=default", headers=_bearer("viewer", "tenant-b"))
    assert resp.status_code == 403
    assert "secret-override-for:default" not in resp.text   # the other tenant's rego must NOT leak


def test_override_read_own_namespace_is_allowed() -> None:
    client, _, _ = _client(rows=[])
    resp = client.get("/api/v1/policy-packs/override?namespace=tenant-b", headers=_bearer("viewer", "tenant-b"))
    assert resp.status_code == 200
    assert resp.json()["namespace"] == "tenant-b"          # own namespace still readable


def test_override_read_admin_any_namespace_is_allowed() -> None:
    client, _, _ = _client(rows=[])
    resp = client.get("/api/v1/policy-packs/override?namespace=default", headers=_bearer("admin", "*"))
    assert resp.status_code == 200
    assert resp.json()["namespace"] == "default"           # admin reads any namespace unchanged
