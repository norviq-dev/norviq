...
def fail_open(monkeypatch: pytest.MonkeyPatch) -> None:
    """Put the SDK in the most permissive posture an operator can choose."""
...
async def test_5xx_blocks_under_the_default_fail_closed_posture(status: int) -> None:
    """Default posture is unchanged: an outage still blocks."""
    assert settings.sdk_fallback_mode == "block"  # the shipped default
    client = _client(_status_transport(status))
    decision = await client.evaluate(_event())
