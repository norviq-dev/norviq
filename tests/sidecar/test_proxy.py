...
from tests.conftest import SeededClusterLoader
...
@pytest.fixture
async def proxy(
    socket_path: str, monkeypatch: pytest.MonkeyPatch, redis_url: str, comprehensive_rego: str
) -> AsyncIterator[SidecarProxy]:
    ...
    # warm_cache() loads whatever the shared local dev Postgres `policies` table happens to contain right
    # now (no fixed/fresh schema per test run), so relying on it for a deterministic verdict is fragile.
    # Seed the real comprehensive.rego as the cluster-wide baseline candidate directly in the loader's
    # in-memory index (mirrors tests/conftest.py's SeededClusterLoader — no DB write needed, since
    # _collect_candidates reads `_loader._policies` directly and _evaluate_opa never falls back to a
    # policies dict once a candidate's rego is passed explicitly).
    sidecar._loader._policies["__cluster__:__baseline__"] = {"rego": comprehensive_rego, "priority": 100}
    yield sidecar
    await sidecar.stop()
...
async def test_http_fallback_allows_safe_tool(
    redis_url: str, monkeypatch: pytest.MonkeyPatch, comprehensive_rego: str
) -> None:
    """HTTP fallback should forward safe requests."""
    cache = RedisCache(url=redis_url)
    await cache.connect()
    evaluator = OPAEvaluator(cache)
    # No loader bound -> _collect_candidates returns [] -> the no-policy path fails CLOSED
    # (no_policy_loaded), which would block even a safe tool. Bind the real comprehensive.rego as the
    # cluster baseline so the verdict reflects an actual policy decision, not the no-policy default.
    evaluator.bind_loader(SeededClusterLoader(comprehensive_rego))
...
async def test_http_fallback_blocks_sql_injection(
    redis_url: str, monkeypatch: pytest.MonkeyPatch, comprehensive_rego: str
) -> None:
    ...
    await cache.connect()
    evaluator = OPAEvaluator(cache)
    # See test_http_fallback_allows_safe_tool: without a bound loader this would fail-closed on
    # no_policy_loaded regardless of the payload, masking whether deny_sql_injection actually fired.
    evaluator.bind_loader(SeededClusterLoader(comprehensive_rego))
    resolver = SPIFFEResolver()
    interceptor = ToolInterceptor(evaluator, resolver)
    emitter = AuditEmitter()
    ...
    # Now that a real policy is bound, pin down WHY it dropped — a bare no_policy_loaded fail-closed
    # would satisfy the action check above too, without proving deny_sql_injection actually fired.
    assert body["decision"]["rule_id"] == "deny_sql_injection"
    await evaluator.close()
    await cache.close()
