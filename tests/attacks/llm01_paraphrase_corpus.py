    ...
    payloads.update(
        {
            ...
            # Natural jailbreaks that omit a temporal context word (observed bypasses).
...
def malicious_base64_corpus() -> list[str]:
    ...
    # Short base64 destructive commands (encoded < 16 chars, within the decode floor).
    for cmd in ("rm -rf /", "cat /etc/shadow", "nc -e /bin/sh"):
        encoded.add(base64.b64encode(cmd.encode("utf-8")).decode("ascii"))
    # Triple-nested base64 of a destructive SQL payload (decode depth > 2).
