...
async def test_audit_stats_reports_avg_latency_ms(
    ...
    assert resp.status_code == 200
    body = resp.json()
    assert "avg_latency_ms" in body, f"avg_latency_ms missing: {body}"
    assert isinstance(body["avg_latency_ms"], (int, float))
    assert body["avg_latency_ms"] >= 0.0
    # if the window has calls, the average latency is a real positive number (records stamp real latency).
    if body.get("total", 0) > 0:
        assert body["avg_latency_ms"] > 0.0, f"stats have {body['total']} calls but avg_latency_ms is 0: {body}"
