...
@cli.group()
def admin() -> None:
    """Admin recovery commands (in-cluster, no email/SMTP)."""


@admin.command("reset-password")
@click.option("--namespace", "-n", default="norviq", show_default=True, help="Kubernetes namespace of the Norviq release.")
@click.option("--context", "kube_context", default=None, help="kubectl context (default: current).")
@click.option("--username", "-u", default="admin", show_default=True, help="Local user to reset.")
@click.option("--console-url", default="http://localhost:8080", show_default=True,
              help="Base URL where you reach the console (for the sign-in hint).")
def admin_reset_password(namespace: str, kube_context: str | None, username: str, console_url: str) -> None:
    """Reset a local user's password IN-CLUSTER — a NO-EGRESS recovery (no email/SMTP).

    kubectl-exec runs the in-pod reset, which sets a fresh ONE-TIME password + forces a change on next
    login. The one-time password is captured from the pod and printed here; nothing leaves the cluster.
    """
    import subprocess  # local import: only needed for this command
    ...
    cmd = kubectl + [
        "-n", namespace, "exec", "deploy/norviq-api", "-c", "api", "--",
        "python", "-m", "norviq.api.admin_reset", "--username", username,
    ]
    try:
        out = subprocess.run(cmd, capture_output=True, text=True, timeout=30)
    except FileNotFoundError:
        ...
        raise click.ClickException("Timed out reaching the api pod. Is the deployment running and your context correct?")
    if out.returncode != 0:
        raise click.ClickException(f"Password reset failed (kubectl exec):\n{out.stderr.strip()}")
    pw = out.stdout.strip().splitlines()[-1].strip() if out.stdout.strip() else ""
    if not pw:
        raise click.ClickException("The api pod returned an empty password.")
    click.echo(f"Reset '{username}'. Sign in with this ONE-TIME password (you will be forced to set a new one):")
    click.echo("")
    click.echo(f"     username: {username}")
    click.echo(f"     password: {pw}")
    click.echo("")
    click.echo(f"  Console: {console_url.rstrip('/')}/login")
    _ok("admin reset-password")
