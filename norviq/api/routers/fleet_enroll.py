...
async def fleet_join(
    ...
    # from an older, unguarded mint — re-check at the point of use.
    try:
        await assert_safe_url_async(hub_url, context="fleet join hub_url")
    except SSRFBlockedError as exc:
        log.warning("nrvq.fleet.join_rejected", error=str(exc), code="NRVQ-FLT-15033")
        raise HTTPException(status_code=422, detail=f"join token hub_url failed the SSRF safety check: {exc}") from exc
