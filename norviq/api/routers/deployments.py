...
async def list_deployments(
    ...
    async for key in cache._client().scan_iter("trust:*"):
        ...
        if (namespace and ns != namespace) or not agent_class or (ns, agent_class) in seen:
            continue
        seen.add((ns, agent_class))
        rows.append({"name": agent_class, "namespace": ns, "agent_class": agent_class})
