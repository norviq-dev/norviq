...
build_load_images() {
  ...
  docker build --platform "$plat" -t norviq/norviq-engine:engine-latest  -f "$REPO_ROOT/Dockerfile.engine"  "$REPO_ROOT"
  docker build --platform "$plat" -t norviq/norviq-engine:webhook-latest -f "$REPO_ROOT/webhook/Dockerfile" "$REPO_ROOT/webhook"
  kind load docker-image --name "$CLUSTER" \
    norviq/norviq-engine:api-latest norviq/norviq-engine:ui-latest \
    norviq/norviq-engine:engine-latest norviq/norviq-engine:webhook-latest
}
